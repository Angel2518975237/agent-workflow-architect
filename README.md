...
Skill 会结合已有信息逐步确认，已知的部分不重复询问：

1. “每次都能按固定步骤和条件分支处理，还是需要看到结果后再决定下一步？”——可预定义路径时优先 Workflow，流程中仍可调用模型。
2. “现在具体卡在哪些情况，有什么质量、时间、费用或权限要求？”——由 Skill 判断一个 Agent 加工具是否足够，证据不足时先做小样例验证。
3. “哪些部分可以独立做，各自产出什么、怎样验收？”——独立且协作收益覆盖成本时考虑 Multi-Agent；紧密依赖但有固定流程骨架时考虑 Agent + Workflow。

需求尚不明确不等于需要 Agent，任务不能拆分也不必然适合混合架构。Workflow 与 Agent 描述执行路径如何决定，Single 与 Multi 描述执行者如何组织；选型可以组合。最后给出推荐、依据、待验证项和下一步。
