...
2. 架构选型先用业务问题判断步骤能否预定义，再判断一个 Agent 加工具是否足够，按 [提问导航](references/agent-architecture.md) 只追问关键缺口。步骤多、工具多不是增加 Agent 的理由；并行拆分要有独立输入、可单独验收的结果和覆盖协调成本的收益。为权限隔离或复核拆分时说明额外成本。
